...
errcode_t ext2fs_open(vnode_t vp, int flags, int superblock,
		      unsigned int block_size, io_manager manager,
		      ext2_filsys *ret_fs)
{
	return ext2fs_open2(vp, 0, flags, superblock, block_size,
			    manager, ret_fs);
}
 ...
 *				filesystems)
 */
errcode_t ext2fs_open2(vnode_t vp, const char *io_options,
		       ...
		       ext2_filsys *ret_fs)
{
	...
	retval = ext2fs_get_mem(pathlen, &fs->device_name);
	if (retval)
		goto cleanup;
    retval = vn_getpath(vp, fs->device_name, &pathlen);
    if (retval)
        goto cleanup;
	...
	retval = manager->open(vp, fs->device_name, io_flags, &fs->io);
	if (retval)
		goto cleanup;
	if (io_options &&
