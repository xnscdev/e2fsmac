...
static void dbg_print_extent(char *desc, struct ext2fs_extent *extent)
{
	if (desc)
		printf("%s: ", desc);
	printf("extent: lblk %llu--%llu, len %u, pblk %llu, flags: %s%s%s%s\n",
	       extent->e_lblk, extent->e_lblk + extent->e_len - 1,
	       extent->e_len, extent->e_pblk,
           extent->e_flags & EXT2_EXTENT_FLAGS_LEAF ? "LEAF " : "",
           extent->e_flags & EXT2_EXTENT_FLAGS_UNINIT ? "UNINIT " : "",
           extent->e_flags & EXT2_EXTENT_FLAGS_SECOND_VISIT ? "2ND_VISIT " : "",
           !extent->e_flags ? "(none)" : "");
}
				   ...
				   int expand_allowed)
{
	...
	if (new_root) {
		...
		ext2fs_extent_get(handle, EXT2_EXTENT_UP, &extent);
...
errcode_t ext2fs_extent_delete(ext2_extent_handle_t handle, int flags)
{
	...
	if (path->entries == 0 && handle->level) {
		if (!(flags & EXT2_EXTENT_DELETE_KEEP_EMPTY)) {
			...
			ext2fs_extent_delete(handle, flags);
